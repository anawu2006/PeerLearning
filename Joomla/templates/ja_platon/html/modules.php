...
{ 
	...
	if (!empty ($module->content)) {
		...
		$html = "<{$moduleTag} class=\"section t3-module {$fullWidthClass} {$moduleClassSfx} {$moduleClass}\" id=\"section-{$module->id}\">" .
					"<div class=\"section-inner\">" . $badge;

		if ($module->showtitle != 0) {
			$html .= "<{$headerTag} class=\"section-title module-title {$headerClass}\"><span>{$module->title}</span>";
		...
		$html .= "<div class=\"section-ct module-ct\">{$module->content}</div></div></{$moduleTag}>";
